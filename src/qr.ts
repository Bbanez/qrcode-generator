...
export class Qr {
    ...
    private getFormatBCH(mask: number): number {
        ...
        while (value.toString(2).length >= generator.toString(2).length) {
            const shift =
                value.toString(2).length - generator.toString(2).length;
    ...
    private buildBits(mask: number, dataBytes: number[], dataMask: Mat): Mat {
        const formatData =
            ((((this.info.ecLevel << 3) | mask) << 10) |
                this.getFormatBCH(mask)) ^
            0b101010000010010;
        const versionData =
            this.info.version >= 7
        ...
        if (this.info.version >= 7) {
            for (let i = 0; i < 18; i++) {
                ...
                bits.data[Math.floor(i / 3)][this.info.size.x - 11 + (i % 3)] =
                    bit;
                bits.data[this.info.size.y - 11 + (i % 3)][Math.floor(i / 3)] =
                    bit;
            }
        }
